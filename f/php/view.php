<div id='view'>
    <section>
        <?php
        $viewcountry = @$_GET['country'];
        $viewstate = @$_GET['state'];
        $viewpostid = @$_GET['postid'];
        include 'connect.php';
//user not logged in
        if ($_SESSION['start'] && !$_SESSION['userid'] && $viewcountry && $viewstate) {


            $feed = "SELECT posttext,time FROM $table2 WHERE status='1' AND postid='$viewpostid' AND country='$viewcountry' AND state='$viewstate'";
            $feedre = mysqli_query($conn, $feed);
            if (mysqli_num_rows($feedre) > 0) {
                // output data of each row
                while ($feedrow = mysqli_fetch_assoc($feedre)) {
                    $copiessql = "SELECT * FROM $table3 WHERE status='0' AND postid='$viewpostid'";
                    $copiesresult = mysqli_query($conn, $copiessql);
                    $copies = mysqli_num_rows($copiesresult);
                    $cut = "SELECT * FROM $table7 WHERE status='0' AND postid='$viewpostid'";
    ...
    if (@!$_COOKIE['cookie_country'] && @!$_COOKIE['cookie_state'] && !$_SESSION['userid']) {
        ...
        while ($feedrow = mysqli_fetch_assoc($feedre)) {
            ...
            if (mysqli_num_rows($copiedresult) == 0) {
                ...
                if (mysqli_num_rows($cutre) == 0) {
                    ...
                    <?php if (@$_GET['cutconfirm'] == $postid) {
                        include 'cutconfirmform.php';
                    } ?>
                                <nav>
                                    <ul id='de'>
                                        <li><?php echo " Copies   :  " . $copies; ?></li>
