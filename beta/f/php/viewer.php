...
include 'connect.php'; 
//user not logged in
		if ($_SESSION['start'] && !$_SESSION['userid'] &&@$_COOKIE['cookie_country']){
		
			$cookiecountry=$_COOKIE['cookie_country'];
			$cookiestate=$_COOKIE['cookie_state'];
			$feed = "SELECT * FROM $table2 WHERE status='1' AND country='$cookiecountry'  ORDER BY time DESC LIMIT 0,50";
